...
------------------------------TRIP_DETAIL AND INSERT ALL INTO ONE ---------------------------------------
...
---------------CREATE TABLE AND INSERT DATA FOR LOCATION -------------------------------
...
from 
(select driverid,sum (FARE) as fares from trips 
group by driverid
order by sum (FARE) desc)b)c;
-- which duration had more trips
select * from trips; 

select top 1 duration ,count(distinct tripid) as tripd from trips 
group by duration 
order by count(distinct tripid) asc;
